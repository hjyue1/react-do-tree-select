 ...
 * @return {[null]}   null
 */
var parentChain = exports.parentChain = function parentChain(TreeDataMap, parentNode, config, checkedList, isInit) {
    if (parentNode) {

        if (isInit && config.initCheckedList.some(function (item) {
            return item.toString() == parentNode.value;
        })) {
            // 如果是初始化状态触发的该函数，初始化勾选列表有当前节点 就不操作
        } else {
            var checkStatus = childCheckedStatus(parentNode.children, TreeDataMap, config, isInit);
